...
PAGE_DIR equ 0x1000
PAGE_TAB1 equ 0x2000
PAGE_TAB2 equ 0x3000
...
start:
    mov eax, PAGE_TAB1 | 0b11   ; present and read/write attributes
    mov dword [PAGE_DIR], eax   ; first entry of the page directory

    ; fill other entries with the address of the second page table
    mov ebx, PAGE_DIR + 4
fill_page_dir:
    mov dword [ebx], PAGE_TAB2 | 0b11 ; present and read/write attributes
    add ebx, 4
    cmp ebx, 0x2000
    jne fill_page_dir

    ; fill the first page table (identity paging)
    mov ebx, PAGE_TAB1
    sub esi, esi
fill_page_tab1:
    mov eax, esi
    or eax, 0b11     ; present and read/write attributes
    mov dword [ebx], eax
    add esi, 0x1000
    add ebx, 4
    cmp ebx, 0x3000
    ...
    mov eax, PAGE_DIR
    mov cr3, eax      ; load cr3 with th address of the page directory
    mov eax, cr0
    or eax, 0x80000001    ; set protected mode and paging
    mov cr0, eax
...
pmode_start:
    ...
    jmp 8:0x4200      ; jump to the kernel (init.c)
